...
export interface Lead { id:string; firstName:string; lastName:string; business:string; phone:string; email:string; whatsapp:string; instagram:string; facebook:string; website:string; businessType:string; city:string; state:string; source:Source; service:Service; stage:Stage; potentialValue:number; preferredLanguage:Language; aiScore?:number; priority?:'critical'|'high'|'medium'|'low'; interestLevel?:'high'|'medium'|'low'; updatedAt?:string; lastContact?:string; nextFollowUp?:string; quoteStatus:'not_sent'|'sent'|'accepted'|'rejected'; quoteAmount?:number; quoteDate?:string; finalSaleAmount?:number; saleDate?:string; lostReason?:string; createdAt:string; demo?:boolean }
...
export interface Task {id:string; prospectId?:string; title:string; description?:string; dueDate:string; priority:'critical'|'high'|'medium'|'low'; status:'open'|'completed'; source:'manual'|'followup'|'automation'|'ai'; createdAt:string; updatedAt?:string}
