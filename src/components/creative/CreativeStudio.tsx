...
export function CreativeStudio({ leads, lang }: { leads: Lead[]; lang: Language }) {
  ...
  return <>
    ...
    {tab === "templates" && <section className="template-grid">{templates.map((template) => <article className="template" key={template}><span>{template}</span><h3>{text(lang, template === "Neutral" ? "Prompt-led neutral composition" : template === "Bold Promotion" ? "High-impact offer layout" : template === "Minimal Business" ? "Refined editorial hierarchy" : template === "Technology Launch" ? "Modern technology composition" : template === "Clean SaaS" ? "Structured clean-copy layout" : "Premium navy campaign treatment", template === "Neutral" ? "Composición neutral dirigida por el prompt" : template === "Bold Promotion" ? "Diseño de oferta de alto impacto" : template === "Minimal Business" ? "Jerarquía editorial refinada" : template === "Technology Launch" ? "Composición tecnológica moderna" : template === "Clean SaaS" ? "Diseño limpio y estructurado" : "Tratamiento premium navy")}</h3><button onClick={() => { update({ template }); setTab("create"); }}>{text(lang, "Use template", "Usar plantilla")}</button></article>)}</section>}
  </>;
}
