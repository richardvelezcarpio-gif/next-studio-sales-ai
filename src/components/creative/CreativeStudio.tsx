...
export function CreativeStudio({ leads, lang }: { leads: Lead[]; lang: Language }) {
  ...
  const help = async () => { const suggestedCta = request.language === "es" ? "Ver Detalles" : "View Details"; const fallback = { hook: request.service ? `${request.service}: built for the next move` : "Make your next move count", supporting: request.service ? `A premium ${request.service.toLowerCase()} experience designed to move business forward.` : "A premium solution designed to move business forward.", cta: request.cta || suggestedCta, visual: `${request.service || "Modern business"} campaign with sophisticated commercial lighting, polished materials, and clear negative space for copy.` }; const result: any = await requestOpenAI("creative_copy", { service: request.service, goal: request.goal, audience: request.audience, language: lang, style: request.style }); const copy = result.result || fallback; update({ hook: copy.hook || fallback.hook, offer: copy.supporting || copy.supportingText || fallback.supporting, cta: copy.cta || fallback.cta, visualDirection: copy.visual || copy.visualDirection || fallback.visual }); };
