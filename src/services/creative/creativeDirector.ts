...
export const createCreativeDirection=(request:CreativeGenerationRequest,brand?:BrandKit|null):CreativeDirection=>{
  ...
  const finalImagePrompt=[`${style} image of ${subject}.`,`Environment: ${category[2]}.`,`Composition: ${composition}.`,`Camera: ${camera}.`,`Lighting: ${lighting}.`,`Details: ${details}.`,`Colors: ${colors}.`,`Negative space: ${negativeSpace}.`,`High-resolution appearance and realistic proportions. Do not change the requested concept or introduce unrelated technology, devices, business imagery, dark palettes, or branding.`,`Avoid: ${avoid.join(", ")}.`].join(" ");
