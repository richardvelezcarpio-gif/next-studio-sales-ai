...
export const followUpsRepository={
  ...
  getByProspectId:async(prospectId:string)=>{const {data,error}=await client().from('follow_ups').select('*').eq('prospect_id',prospectId).order('created_at',{ascending:false});if(error)throw error;return(data||[]).map(map)},
